...
HRESULT CLevel_WhiteMage::Initialize()
{
	...
	CGameInstance::Get_Instance()->PlaySoundW(L"EndingBGM.mp3", 31, 1.f);
...
HRESULT CLevel_WhiteMage::Ready_Layer_Npc(const _tchar * pLayerTag)
{
	...
	if (FAILED(pGameInstance->Add_GameObjectToLayer(TEXT("Prototype_GameObject_ElderStan"), LEVEL_DARKMAGE, pLayerTag)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObjectToLayer(TEXT("Prototype_GameObject_Bulb"), LEVEL_DARKMAGE, pLayerTag)))
		return E_FAIL;
